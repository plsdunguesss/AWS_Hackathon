...
class SafetyMonitorService {
    ...
    async scanForHarmfulContent(message) {
        ...
        const crisisMatches = this.crisisKeywords.filter(keyword => lowerMessage.includes(keyword));
        ...
        const harmMatches = this.harmKeywords.filter(keyword => lowerMessage.includes(keyword));
        ...
        const concerningMatches = concerningPatterns.filter(pattern => lowerMessage.includes(pattern));
    ...
    async preventHarmfulSuggestions(aiResponse) {
        ...
        const containsHarmful = harmfulSuggestions.some(suggestion => lowerResponse.includes(suggestion));
        if (containsHarmful) {
