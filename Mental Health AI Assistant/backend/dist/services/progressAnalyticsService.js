...
exports.ProgressAnalyticsService = void 0;
...
class ProgressAnalyticsService {
    ...
    async performCorrelationAnalysis(sessionId, timeRange = '30d') {
        ...
        for (const pair of correlationPairs) {
            const xValues = moodHistory.map(e => e[pair.x]);
            const yValues = moodHistory.map(e => e[pair.y]);
    ...
    async detectPatterns(sessionId, timeRange = '30d') {
        const days = this.parseTimeRange(timeRange);
        const moodHistory = await this.dashboardService.getMoodHistory(sessionId, days);
    ...
    async trackMilestones(sessionId) {
        const milestones = [];
    ...
    async generateVisualizationData(sessionId, timeRange = '30d') {
        const days = this.parseTimeRange(timeRange);
        const moodHistory = await this.dashboardService.getMoodHistory(sessionId, days);
    ...
    async getAdvancedAnalytics(sessionId, timeRange = '30d') {
    ...
    determineSignificance(correlation, sampleSize) {
        // Statistical significance based on correlation strength and sample size
        if (sampleSize < 10)
            return 'low';
        if (correlation > 0.7 && sampleSize >= 20)
            return 'high';
        if (correlation > 0.5 && sampleSize >= 15)
            return 'medium';
        if (correlation > 0.3 && sampleSize >= 10)
            return 'low';
        return 'low';
    }
    generateCorrelationDescription(title, correlation, significance) {
    ...
    generateCorrelationInsight(x, y, correlation) {
        const insights = {
            'sleep': {
    ...
    async detectTrendPatterns(moodHistory) {
        if (moodHistory.length < 7)
            return [];
        const patterns = [];
        const metrics = ['mood', 'energy', 'stress', 'anxiety', 'sleep'];
        for (const metric of metrics) {
            const values = moodHistory.map(e => e[metric]);
    ...
    async detectCyclicalPatterns(moodHistory) {
    ...
    async detectAnomalies(moodHistory) {
        if (moodHistory.length < 10)
            return [];
        const patterns = [];
        const metrics = ['mood', 'energy', 'stress', 'anxiety', 'sleep'];
    ...
    async detectActivityPatterns(moodHistory, activities) {
        const patterns = [];
    ...
    async detectConsistencyPatterns(moodHistory) {
        if (moodHistory.length < 7)
            return [];
        const patterns = [];
        const metrics = ['mood', 'energy', 'stress', 'anxiety', 'sleep'];
    ...
    async trackMoodMilestones(sessionId) {
    ...
    async trackConsistencyMilestones(sessionId) {
    ...
    async trackActivityMilestones(sessionId) {
    ...
    async trackRiskReductionMilestones(sessionId) {
        const milestones = [];
        // Get recent risk assessments
        const recentAssessments = await this.db.all(`SELECT overall_risk, created_at FROM risk_assessments 
       WHERE session_id = ? 
    ...
    async generateDataInsights(moodHistory) {
        const insights = [];
    ...
    async generateRecommendations(sessionId, moodHistory) {
    ...
    async analyzeTrends(sessionId, timeRange) {
        const days = this.parseTimeRange(timeRange);
        const moodHistory = await this.dashboardService.getMoodHistory(sessionId, days);
        const trends = [];
        const metrics = ['mood', 'energy', 'stress', 'anxiety', 'sleep'];
    ...
    async identifyRiskFactors(sessionId, timeRange) {
        const days = this.parseTimeRange(timeRange);
        const moodHistory = await this.dashboardService.getMoodHistory(sessionId, days);
        const riskFactors = [];
        if (moodHistory.length === 0)
    ...
    detectWeeklyPattern(moodHistory) {
        const dayAverages = {
    ...
    estimateCompletion(current, target) {
        if (current >= target)
...
exports.ProgressAnalyticsService = ProgressAnalyticsService;
//# sourceMappingURL=progressAnalyticsService.js.map