...
export class UserService {
   ...
   */
  async upsertUserProfile(sessionId: string, profileData: Partial<Omit<UserProfile, 'id' | 'sessionId' | 'createdAt' | 'updatedAt'>>): Promise<UserProfile> {
    try {
      ...
      if (existingProfile) {
        ...
        return await this.getUserProfile(sessionId) as UserProfile;
      } else {
        // Create new profile
   ...
   */
  async upsertUserPreferences(sessionId: string, preferencesData: Partial<Omit<UserPreferences, 'id' | 'sessionId' | 'createdAt' | 'updatedAt'>>): Promise<UserPreferences> {
    try {
      ...
      if (existingPreferences) {
        ...
        return await this.getUserPreferences(sessionId) as UserPreferences;
      } else {
   ...
   */
  async hasUserProfile(sessionId: string): Promise<boolean> {
    try {
   ...
   */
  async hasUserPreferences(sessionId: string): Promise<boolean> {
    try {
